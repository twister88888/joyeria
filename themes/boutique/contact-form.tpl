...
<div class="row">
	<div class="col-sm-6">
		<div class="kt-contact-form margin-top-60">
			<h3 class="title">{l s='REACH US FOR ANY QUESTIONS YOU MIGHT HAVE'}</h3>
				...
				<ul class="footer_links clearfix">
					<li>
						<a class="btn btn-default button button-small" href="{if isset($force_ssl) && $force_ssl}{$base_dir_ssl}{else}{$base_dir}{/if}">
							<span>
								<i class="fa fa-chevron-left"></i>{l s='Home'}
							</span>
						</a>
					</li>
				</ul>
						...
						<div class="clearfix">
								...
								{if isset($customerThread.id_contact) && $customerThread.id_contact && $contacts|count}
										...
										{foreach from=$contacts item=contact}
											{if $contact.id_contact == $customerThread.id_contact}
												<input type="text" class="form-control" id="contact_name" placeholder="{l s='Your name'}" name="contact_name" value="{$contact.name|escape:'html':'UTF-8'}" readonly="readonly" />
												<input type="hidden" name="id_contact" value="{$contact.id_contact|intval}" />
												{$flag=false}
											{/if}
										{/foreach}
										{if $flag && isset($contacts.0.id_contact)}
												<input type="text" class="form-control" id="contact_name" placeholder="{l s='Your name'}" name="contact_name" value="{$contacts.0.name|escape:'html':'UTF-8'}" readonly="readonly" />
								...
								<p class="form-group">
									...
									{if isset($customerThread.email)}
										<input class="form-control" type="text" id="email" placeholder="{l s='Email address'}" name="from" value="{$customerThread.email|escape:'html':'UTF-8'}" readonly="readonly" />
									{else}
										<input class="form-control validate" type="text" placeholder="{l s='Email address'}" id="email" name="from" data-validate="isEmail" value="{$email|escape:'html':'UTF-8'}" />
									{/if}
								</p>
								{if !$PS_CATALOG_MODE}
									{if (!isset($customerThread.id_order) || $customerThread.id_order > 0)}
										<div class="form-group selector1">
											...
											{elseif !isset($customerThread.id_order) && empty($is_logged)}
												<input class="form-control" placeholder="Order reference" type="text" name="id_order" id="id_order" value="{if isset($customerThread.id_order) && $customerThread.id_order|intval > 0}{$customerThread.id_order|intval}{else}{if isset($smarty.post.id_order) && !empty($smarty.post.id_order)}{$smarty.post.id_order|escape:'html':'UTF-8'}{/if}{/if}" />
											{elseif $customerThread.id_order|intval > 0}
												<input class="form-control" placeholder="Order reference" type="text" name="id_order" id="id_order" value="{if isset($customerThread.reference) && $customerThread.reference}{$customerThread.reference|escape:'html':'UTF-8'}{else}{$customerThread.id_order|intval}{/if}" readonly="readonly" />
											{/if}
										</div>
									{/if}
									{if isset($is_logged) && $is_logged}
										<div class="form-group selector1">
											...
											{elseif $customerThread.id_product > 0}
												<input  type="hidden" placeholder="{l s='Product'}" name="id_product" id="id_product" value="{$customerThread.id_product|intval}" readonly="readonly" />
											{/if}
										</div>
									{/if}
								{/if}
								...
								<div class="form-group">
									...
									<textarea class="form-control" placeholder="{l s='Your message!'}" id="message" name="message">{if isset($message)}{$message|escape:'html':'UTF-8'|stripslashes}{/if}</textarea>
								</div>
						</div>
						<div class="submit">
							<button type="submit" name="submitMessage" id="submitMessage" class="button">{l s='SEND MESSAGE'}</button>
						</div>
					</fieldset>
				</form>
			{/if}
	...
	<div class="col-sm-6">
		<div class="margin-top-60">
			<div class="google-map">{hook h='displayStoreMap'}</div>
