...
{if !$opc}
			<p class="cart_navigation clearfix">
				...
				{if $back}
					<a href="{$link->getPageLink('order', true, NULL, "back={$back}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button-exclusive btn btn-default"><i class="fa fa-chevron-left"></i>{l s='Continue Shopping'}</a>
				{else}
					<a href="{$link->getPageLink('order', true, NULL)|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button-exclusive btn btn-default"><i class="fa fa-chevron-left"></i>{l s='Continue Shopping'}</a>
				{/if}
		        <button type="submit" name="processAddress" class="button btn btn-default button-medium"><span>{l s='Proceed to checkout'}<i class="fa fa-chevron-right right"></i></span></button>
			</p>
		</form>
{else}
	</div>
