			...
			<div>
									...
									{if $filter.type == $filter_type && isset($filter.values)}
										{if isset($filter.slider)}
											{if $smarty.foreach.f_values.first}
												<li>
													<a href="#" data-rel="layered_{$filter.type}_slider" title="{l s='Cancel' mod='blocklayered'}"><i class="pe-7s-close"></i></a>
													{if $filter.format == 1}
										...
										{else}
											{foreach from=$filter.values key=id_value item=value}
												{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
													<li>
														<a href="#" data-rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blocklayered'}"><i class="fa fa-remove"></i></a>
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						...
						{else}
							<div class="layered_filter widget_layered_nav clearfix">
						{/if}
						...
						<ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="col-lg-12 layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if} {if isset($filter.slider) && $filter.slider} ul_slider_type{/if}">
							{if !isset($filter.slider)}
							...
							{if count($filter.values) > $filter.filter_show_limit && $filter.filter_show_limit > 0 && $filter.filter_type != 2}
								...
								<span class="hide-action less">{l s='Show less' mod='blocklayered'}</span>         </li>
							{/if}
						</ul>
					</div>
					{/if}
