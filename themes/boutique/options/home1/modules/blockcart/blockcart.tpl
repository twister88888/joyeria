...
<div class="shopping_cart_container style1 clearfix{if $PS_CATALOG_MODE} header_user_catalog{/if}">
{/if}
	...
	<div class="shopping_cart mini-cart">
		<a class="cart-link" href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
			{l s='My cart' mod='blockcart'}
			<span class="ajax_cart_quantity{if $cart_qties == 0} unvisible my_visiable{/if}">{$cart_qties}</span>
			<span class="ajax_cart_product_txt{if $cart_qties != 1} unvisible{if $cart_qties == 0} my_visiable{/if}{/if}">{l s='Item - ' mod='blockcart'}</span>
			<span class="ajax_cart_product_txt_s{if $cart_qties < 2} unvisible my_visiable{/if}">{l s='Items - ' mod='blockcart'}</span>
				<span class="ajax_cart_total{if $cart_qties == 0} unvisible my_visiable{/if}">
					{if $cart_qties > 0}
						{if $priceDisplay == 1}
			...
			<div class="cart_block block exclusive">
				<div class="block_content">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
						{if $products}
							<h3 class="title">{l s='You have ' mod='blockcart'}<span class="text-primary">(<span class="ajax_cart_quantity">{$cart_qties}</span> {l s='items' mod='blockcart'})</span> {l s='in your cart' mod='blockcart'}</h3>
							<dl class="products">
								{foreach from=$products item='product' name='myLoop'}
									...
									<dt data-id="cart_block_product_{$product.id_product|intval}_{if $product.id_product_attribute}{$product.id_product_attribute|intval}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|intval}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
										<div class="thumb">
                                			<a class="cart-images" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}"><img src="{$link->getImageLink($product.link_rewrite, $product.id_image, $cart_img_type)}" alt="{$product.name|escape:'html':'UTF-8'}" /></a>
                                		</div>
                                		<div class="info">
                                			<div class="product-name"><a class="cart_block_product_name" href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">{$product.name|truncate:13:'...'|escape:'html':'UTF-8'}</a></div>
                                			...
                                			<span class="remove_link">
												{if !isset($customizedDatas.$productId.$productAttributeId) && (!isset($product.is_gift) || !$product.is_gift)}
													<a class="remove-item ajax_cart_block_remove_link" href="{$link->getPageLink('cart', true, NULL, "delete=1&id_product={$product.id_product|intval}&ipa={$product.id_product_attribute|intval}&id_address_delivery={$product.id_address_delivery|intval}&token={$static_token}")|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='remove this product from my cart' mod='blockcart'}"><i class="fa fa-close"></i></a>
												{/if}
											</span>
                                		</div>
									</dt>
						...
						<p class="cart-buttons">
							<a class="button" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
									<span>
										{l s='Shopping Cart' mod='blockcart'}
									</span>
							</a>
							<a class="check-out button" href="{$link->getPageLink('order', true, NULL, 'step=1')|escape:'html':'UTF-8'}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
									<span>
		...
		<div class="clearfix">
			...
			<div class="layer_cart_cart col-xs-12 col-md-6">
				...
				<div class="button-container">
					<span class="continue button exclusive-medium" title="{l s='Continue shopping' mod='blockcart'}">
						<span>
							<i class="fa fa-chevron-left left"></i>{l s='Continue shopping' mod='blockcart'}
						</span>
					</span>
					<a class="button button-medium"	href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout' mod='blockcart'}" rel="nofollow">
						<span>
